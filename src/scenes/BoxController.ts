...
this.stateMachine.update(dt);
    ...
    private spawnItems() {
        // Spawn mushroom
    const coordinates = this.scene.matter.add.sprite(this.sprite.x, this.sprite.y - 300, 'coordinates', 0);
    coordinates.setData('type', 'coordinates'); // Set the type data attribute
    // coordinates.setVelocityY(-10);

    // Spawn coin
    const id = this.scene.matter.add.sprite(this.sprite.x, this.sprite.y - 300, 'id', 0);
