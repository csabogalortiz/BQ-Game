...
export default class LevelComplete extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('gifImage', '/assets/Player-Celebrate1.gif'); // GIF image
    ...
    create () {
        ...
        const {width, height} = this.scale 
            ...
            backgroundImage.setDepth(-1);
              ...
              // Create the GIF image sprite
    const gifImage = this.add.image(width * 0.5, height * 0.3, 'gifImage');

    // Set the origin of the GIF image
    gifImage.setOrigin(0.5);
    ...
    // Boton!
    const button = this.add.rectangle(width * 0.5, height * 0.80, 150, 75, 0x8AFFC1)
